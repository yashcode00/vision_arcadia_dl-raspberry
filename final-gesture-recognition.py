...
# CATEGORIES = ['A', 'B', 'C', 'F', 'G','G_invert' 'L', 'M', 'O', 'Q', 'V', 'Y']
    ...
    while True:
        ...
        if ret:
            ...
            if count % 20 == 0:
                ...
                cv2.imwrite("frame%d.jpg" % img_counter, roi)
                category, confidence = predict(model,
                                               "frame%d.jpg" % img_counter)
                # display the information
                font = cv2.FONT_HERSHEY_SIMPLEX
                out_text = "I guess it is "+category + \
                    " "+" ("+str(confidence)+")"
                cv2.putText(frame, out_text, (5, 30), font,
                ...
                if category == "V":
                    ...
                    if not os.path.isdir(path):
                        os.mkdir(path)
                    pag.screenshot("screenshots/screenshot%d.jpg" %
                                   img_counter)
                else:
                    runfunc(category)
                # 3.print("Current File %d \r" % img_counter, end='')
                os.remove("frame%d.jpg" % img_counter)
                img_counter += 1
                count = 0
            count += 1
        else:
